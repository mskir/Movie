...
public class onepieceSeatController {
...
public void go2Ticket(ActionEvent event) {
    try {
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/FXML/onepieceConfirm.fxml"));

        Parent root = loader.load();
        onepieceConfirmController ticketController = loader.getController();

        if (selectedSeatButton != null) {
            String selectedSeat = selectedSeatButton.getText();
            ticketController.setSelectedSeatLabel(selectedSeat);
        }

        if (selectedTime != null) {
            ticketController.setSelectedTimeLabel(selectedTime);
        }
